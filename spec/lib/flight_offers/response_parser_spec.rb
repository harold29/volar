...
RSpec.describe FlightOffers::ResponseParser do
  ...
  it 'parses the response and creates the flight offers' do
    ...
    expect(flight_offers.first.source).to eq('GDS')
    ...
    expect(flight_offers.first.confirmed).to eq(false)
    expect(flight_offers.first.validating_airline_codes).to eq(['TK'])
    expect(flight_offers.first.pricing_options_fare_type).to eq(['PUBLISHED'])
    expect(flight_offers.first.pricing_options_included_checked_bags_only).to eq(true)
    expect(flight_offers.first.pricing_options_refundable_fare).to eq(nil)
    expect(flight_offers.first.pricing_options_no_restriction_fare).to eq(nil)
    expect(flight_offers.first.pricing_options_no_penalty_fare).to eq(nil)
